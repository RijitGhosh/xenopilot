...
async function sendCallback(url: string, payload: {
  communicationId: string;
  type: 'SENT' | 'DELIVERED' | 'OPENED' | 'CLICKED' | 'CONVERTED' | 'FAILED';
  timestamp: string;
  metadata?: any;
}) {
  try {
...
function simulateMessageLifecycle(body: SendRequestBody) {
  const { channel, communicationId, crmCallbackUrl = 'http://localhost:5000/api/receipts' } = body;

  const triggerEvent = (type: 'SENT' | 'DELIVERED' | 'OPENED' | 'CLICKED' | 'CONVERTED' | 'FAILED', delay: number, metadata?: any) => {
    setTimeout(async () => {
  ...
  if (channel === 'RCS') openRate = 0.70;
  if (channel === 'SMS') openRate = 0.80;
  ...
  if (channel === 'RCS') clickRate = 0.25;
  if (channel === 'SMS') clickRate = 0.08;
...
app.post('/send', (req: Request, res: Response) => {
  const { recipient, message, channel, communicationId } = req.body as SendRequestBody;
