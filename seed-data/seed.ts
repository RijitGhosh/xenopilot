...
async function main() {
  ...
  // We want:
  // - ~15 VIP users (high spend, many orders, active or recent)
  // - ~35 Dormant users (no purchase in last 90+ days)
  // - ~50 Active/Regular users (recent purchases, moderate spend)
  ...
  for (const customer of customers) {
    ...
    if (customer.segment === 'VIP') {
      ...
      orderEndRange = ninetyDaysAgo; // last order must be > 90 days ago
    } else { // Active
      ordersToCreate = randomIntRange(2, 7);
      orderStartRange = ninetyDaysAgo; // mostly active recently
    }

    let customerTotalSpend = 0;
    let customerLastOrderDate: Date | null = null;

    for (let o = 0; o < ordersToCreate; o++) {
      if (totalOrderCount >= 500 && customer.segment !== 'VIP') {
        // cap at 500 orders globally, but prioritize VIPs
        continue;
      }
  ...
  for (let c = 0; c < 20; c++) {
    ...
    const campaign = await prisma.campaign.create({
      data: {
        ...
        cta: 'Claim Discount',
        status: c < 4 ? 'LAUNCHED' : 'COMPLETED', // a few active, most completed
  ...
  // Seed message history for completed/launched campaigns
  for (const campaign of spawnedCampaigns) {
    ...
    const matchingCustomers = customers.filter(cust => cust.segment === targetSegment).slice(0, 25); // cap per campaign to keep seed fast

    for (const customer of matchingCustomers) {
      // Determine state flow
      // We want some sent, delivered, opened, clicked, converted, and failed
      const rand = Math.random();
      let status = 'SENT';
      if (rand > 0.05) status = 'DELIVERED';
      if (rand > 0.25) status = 'OPENED';
      if (rand > 0.55) status = 'CLICKED';
      if (rand > 0.75) status = 'CONVERTED';
      if (rand < 0.03) status = 'FAILED';
