...
export class AIService {
  ...
  static async generateCampaign(goal: string, customerCount: number = 100): Promise<GeneratedCampaign> {
...
You must return a JSON object strictly matching this schema:
{
  ...
  static async generateCustomerSummary(customer: {
    name: string;
    totalSpend: number;
    ordersCount: number;
    segment: string;
    lastOrderDate: Date | null;
  }, orderHistory: any[], communicationHistory: any[]): Promise<string> {
    if (openai) {
      try {
        const response = await openai.chat.completions.create({
          model: 'gpt-4o-mini',
  ...
  static async generateAnalyticsInsights(sent: number, delivered: number, opened: number, clicked: number, converted: number, revenue: number): Promise<string[]> {
    if (openai) {
      try {
        const response = await openai.chat.completions.create({
          model: 'gpt-4o-mini',
